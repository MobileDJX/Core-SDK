...
public class BottomSliderAd {
    static PopupWindow pop;

    private static final String TAG = "Interstitial_image";

    @SuppressLint("SetJavaScriptEnabled")
    public static void show(Context context, BottomSliderAdListener listener) {
        try {
            ...
            if (HtmlCode.length() > 0) {
                ...
                listener.onAdsAdShown();
                close.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        listener.onAdsAdDismissed();
                        pop.dismiss();
                    }
                });
