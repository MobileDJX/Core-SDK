...
//        suggestionPart = findViewById(R.id.suggestionPart);
        ...
        ((ImageView) findViewById(R.id.img_close_btn)).setOnClickListener(view -> {
            LoadInterstitial.int_show_listener.onAdsDismissed();
            playerView.onPause();
...
//        Glide.with(this).load(logoURL).into(logo);
    ...
    private void initializePlayer() {
        ...
        SharedPreferences sharedPreferences = getApplicationContext().getSharedPreferences("InterstitialVideo", MODE_PRIVATE);

        String ad_url = sharedPreferences.getString("InterstitialVideo_URL", "");

        String screenType = sharedPreferences.getString("screenType", "");
        ...
        player.addListener(new Player.Listener() {
            ...
            public void onPlaybackStateChanged(int playbackState) {
                ...
                if (playbackState == Player.STATE_ENDED) {
                    LoadInterstitial.int_show_listener.onAdsShowComplete();
                } else if (playbackState == Player.EVENT_PLAYER_ERROR) {
                    LoadInterstitial.int_show_listener.onAdsShowFailure();
...
//                    suggestionPart.setVisibility(View.VISIBLE);
            ...
            public void onPlayerError(PlaybackException error) {
                ...
                com.google.android.exoplayer2.util.Log.e("Player_Error", "Error Code Name:" + error.getErrorCodeName());


                if (error.getErrorCodeName().equals("ERROR_CODE_IO_FILE_NOT_FOUND")) {
                    player.stop();
                    adsLoader.setPlayer(null);
                    playerView.setPlayer(null);
                    ...
                    finish();
                    getWindow().clearFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
                    Toast.makeText(InterstitialLoadActivity.this, "Ad Delivery finish", Toast.LENGTH_SHORT).show();
                } else {
                    throw new IllegalStateException("Unexpected value: " + error.getErrorCodeName());
                }
            }
...
//        player.addAnalyticsListener(new EventLogger());
    ...
    public void onStart() {
        ...
        if (Util.SDK_INT > 23) {
            ...
            if (playerView != null) {
                playerView.onResume();
                LoadInterstitial.int_show_listener.onAdsShowStart();
            }
        }
    }
